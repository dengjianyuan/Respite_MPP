...
def run():
    ...
    model = GCN(task_setting, num_layer=args.num_layers, emb_dim=args.emb_dim, feat_dim=args.feat_dim, drop_ratio=args.drop_ratio, pool=args.pooling).to(device)

    # initialize molecular property prediction experiment in wandb
    wandb.init(project="aidd_MPP", name='GCN_{}_{}_{}'.format(args.mol_prop, args.split_type, args.seed), entity="dengjianyuan", save_code=True)
    ...
    test_model = GCN(task_setting, num_layer=args.num_layers, emb_dim=args.emb_dim, feat_dim=args.feat_dim, drop_ratio=args.drop_ratio, pool=args.pooling).to(device)
    ...
    test_result_df['mol_prop'], test_result_df['model_name'], test_result_df['split_type'], test_result_df['fold'] = args.mol_prop, 'GCN', args.split_type, args.seed
    test_result_df['mol_rep'] = 'Graph'

    save_path = '../results/raw_predictions/GCN/{}/{}/{}'.format(task, args.mol_prop, args.split_type)
